...
function createSyntheticEvent(Interface: EventInterfaceType) {
  /**
   * Synthetic events are dispatched by event plugins, typically in response to
   * a top-level event delegation handler.
   *
   * These systems should generally use pooling to reduce the frequency of
   * garbage collection. The system should check `isPersistent` to determine
   * whether the event should be released into the pool after being dispatched.
   * Users that need a persisted event should invoke `persist`.
   *
   * Synthetic events (and subclasses) implement the DOM Level 3 Events API by
   * normalizing browser quirks. Subclasses do not necessarily have to
   * implement a DOM interface; custom application-specific events can also
   * subclass this.
   */
