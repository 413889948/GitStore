 ...
 *
 * Creates representative browser event sequences for high-level gestures based
 * on pointers. This allows unit tests to be written in terms of simple pointer
 * interactions while testing that the responses to those interactions account
 * for the complex sequence of events that browsers produce as a result.
 *
 * Every time a new pointer touches the surface a 'touchstart' event should be
 * dispatched.
 ...
 *
 * Every time an existing pointer moves a 'touchmove' event should be
 * dispatched.
 * - 'changedTouches' contains the updated touch.
 *
 * Every time an existing pointer leaves the surface a 'touchend' event should
 * be dispatched.
 * - 'changedTouches' contains the released touch.
 * - 'targetTouches' contains any of the remaining active pointers for the
 * target.
 */
...
/**
 * This function should be called after each test to ensure the touchStore is
 * cleared in cases where the mock pointers weren't released before the test
 * completed
